...
const
  crcTables = block:
    ...
    for i in 0u32 ..< 256:
      c = i
      for j in 0 ..< 8:
        c = (c shr 1) xor ((c and 1) * 0xedb88320u32)
      tables[0][i] = c
    for i in 0 ..< 256:
...
func crc32*(v: uint32, data: string): uint32 =
  result = v

  var pos: int
  while data.len() - pos >= 8:
    let
      one = read[uint32](data, pos) xor result
      two = read[uint32](data, pos + 4)
    result =
